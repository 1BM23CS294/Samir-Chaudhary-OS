...
void insert(p* arr, int n) {
    for (int i = 0; i < n; i++) {
        scanf("%d %d %d %d", &arr[i].pid, &arr[i].at, &arr[i].bt, &arr[i].priority);
    }
}
...
void sort(p* arr, int n) {
    for (int i = 0; i < n - 1; i++) {
        for (int j = i + 1; j < n; j++) {
            if (arr[i].at > arr[j].at || (arr[i].at == arr[j].at && arr[i].priority > arr[j].priority)) {
                p temp = arr[i];
                arr[i] = arr[j];
                arr[j] = temp;                                                              
            }
...
int main() {
    ...
    printf("Enter Process ID, Arrival Time, Burst Time, and Priority for Processes: ");
    insert(arr, n);
